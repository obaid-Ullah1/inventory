...
export default function Notification_Settings() {
  const settings = [
    ...
    },
    {
      ...
      description: 'Receive notification of Upcoming Appointments',
    },
    {
      heading: 'Urgent Observation',
      description: 'Patient requested an urgent observation',
    },
    {
      heading: 'Low Inventory Alert',
      description: 'Get notified when a product quantity is low or out of stock',
    },
  ];
    ...
    <div className="py-6">
      ...
      <div className="flex flex-wrap mt-10 mb-5 gap-y-5 md:gap-10">
        {settings.map((content, index) => (
          <div
            key={index}
            className="space-y-2 p-3 sm:p-5 bg-white shadow-sm shadow-gray-300 rounded-lg max-w-[30rem] w-full flex items-center justify-between"
          >
