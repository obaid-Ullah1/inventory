...
export default function SideMenu({ sideBar, toggleSideBar }) {
  ...
  const MenuItems = useMemo(() => [
    ...
    { id: 1, icon: <BsClockFill />, label: "Appointment", path: "/Appointment" },
    { id: 2, icon: <IoMdQrScanner />, label: "Scanner", path: "/Scanner" },
    { id: 3, icon: <IoChatboxSharp />, label: "Messages", path: "/Messages" },
    { id: 4, icon: <FaClipboard />, label: "Patient Record", path: "/PatientRecord" },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div ref={MenuRef} className={`bg-gradient-to-b flex flex-col justify-between from-[#1AADAE] to-[#128C82] fixed top-0 left-0 md:top-5 md:left-5 z-10 rounded-none md:rounded-lg h-[100vh] md:h-[95%] w-[16rem] md:w-[14rem] lg:w-[16rem] xl:w-[18rem] transition-transform duration-500 ease-in-out transform ${sideBar ? "translate-x-0 opacity-100 scale-100" : "-translate-x-full opacity-100 scale-100"} md:translate-x-0`}>
              ...
              <Link
                ...
                className={`flex items-center px-6 gap-x-3 rounded-full cursor-pointer py-2 transition-all duration-500 transform ${
                  location.pathname === item.path
                    ? "bg-white text-[#1AADAE] scale-110 shadow-lg" // Active
                    : "hover:text-[#1AADAE] hover:bg-white hover:scale-105" // Hover
                }`}
              >
        ...
        {/* Logout Button */}
        <div
          className="flex mx-3 text-[0.9rem] lg:text-[1rem] mb-14 sm:mb-6 items-center font-medium text-white px-6 gap-x-3 rounded-full hover:text-[#128C82] hover:bg-white py-2 cursor-pointer transform transition-all duration-500 hover:scale-110"
